...
def parse_args():
    ...
    parser.add_argument(
        '--target-layers',
        default=['neck.fpn_convs[3]'],
        nargs='+',
        type=str,
        help='The target layers to get CAM, if you have multiple layers, '
        'take the average , if not set, the tool will '
...
def init_model_cam(args, cfg):
    ...
    if args.preview_model:
        ...
        print('\n Please remove `--preview-model` t-o get the CAM.')
        return
...
def main():
    ...
    for image_path in images:
        if image_path.endswith('.jpg') or image_path.endswith('.png'):
            ...
            # 获取CAM，在下类中又额外进行了一次forward和backward(grad-based)
            # 并进行了归一化、调整到图像大小
            ...
            print(image_with_bounding_boxes.shape)

            if args.out_dir:
                mmcv.mkdir_or_exist(args.out_dir)
                out_file = os.path.join(args.out_dir, "CAM_output.png")
                ...
                cv2.imshow(os.path.basename(image_path), grayscale_cam)
                cv2.waitKey(0)
