...
TransferTrainPage::TransferTrainPage(Ui_MainWindow *main_ui, BashTerminal *bash_terminal,
 DatasetInfo *globalDatasetInfo, ModelInfo *globalModelInfo, TorchServe *globalTorchServe, ModelDock *modelDock):
    ...
    connect(ui->action_importDataset_BBOX, &QAction::triggered, this, &TransferTrainPage::refreshSARDataset);
    connect(ui->action_Delete_dataset, &QAction::triggered, this, &TransferTrainPage::refreshSARDataset);
...
void TransferTrainPage::refreshOpticalDataset(){
    // 更新下拉选择框
    ui->opticalDataBox->clear();
    std::vector<std::string> comboBoxContents = datasetInfo->getNamesInType(optivalDataType.toStdString());
    for(auto &item: comboBoxContents){
        ui->opticalDataBox->addItem(QString::fromStdString(item));
    }
}

void TransferTrainPage::refreshSARDataset(){
    // 更新下拉选择框
    ui->sarDataBox->clear();
    std::vector<std::string> comboBoxContents = datasetInfo->getNamesInType(sarDataType.toStdString());
    for(auto &item: comboBoxContents){
...
void TransferTrainPage::startNormalTrain(){
    ...
    volume = ui->trainNumBox->currentText();
    ...
    if(batchsizes[0]=="" || epochs[0]=="" || lrs[0]=="" || saveModelNames[0]==""){
    ...
    " python ../api/bash/mmdetection/GUI/train_model.py --base_cfg_type FewShot --time "+times[0]+ \
...
void TransferTrainPage::startTransferTrain(){
    ...
    if(ui->useExistedModelBox->isChecked()){
        choicedPreModel = "../db/models/doship_12epoch.pth";
        ...
        if(batchsizes[1]=="" || epochs[1]=="" || lrs[1]=="" ||  batchsizes[2]=="" || 
            epochs[2]=="" || lrs[2]=="" || saveModelNames[1]==""){
            QMessageBox::warning(NULL,"错误","请检查各项文本框中训练参数是否正确配置!");
            return;
        }
        ui->startTransferTrainButton->setEnabled(false);
...
void TransferTrainPage::execuCmd(int modeltypeId, QString cmd){
    ...
    processTrain[modeltypeId]->setProcessChannelMode(QProcess::MergedChannels);
